...
export default function ListOrder(props) {
  ...
  useEffect(() => {
    const fetchGetAll = async () => {
      try {
        const action = await orderApi.getAllOrder(id);
      ...
      case 1:
        return (
          <CustomizedDialogs
            ...
            children={
              <Print>
                <OrderItem id={idReceipt}></OrderItem>
              </Print>
            }
          ></CustomizedDialogs>
        );
      case 2:
        return (
          <CustomizedDialogs
            open={open}
            handleClose={handleClose}
            children={<OrderUpdate id={idReceipt}></OrderUpdate>}
          ></CustomizedDialogs>
        );
      case 3:
        return (
  ...
  const onSubmitDelete = () => {
    const fetchDelete = async () => {
      try {
        const action = await orderApi.deleteOrder(isDelete.id);
